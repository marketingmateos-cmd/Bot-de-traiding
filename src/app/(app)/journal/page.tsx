  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">Every closed trade with its full reproducibility snapshot and automatic post-mortem classification.</p>
      </div>

      {trades.length === 0 && (
        <Card>
          <p className="text-sm text-muted">No trades yet. Run a Paper Trading scan and let positions hit their stop/target.</p>
        </Card>
      )}
        ...
        {trades.map((t) => (
          <Card key={t.id} className="!p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
              ...
              <div>
                <div className="text-muted">Fees / Slippage</div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <Badge tone="muted">Exit: {t.exitReason}</Badge>
              {t.journal?.postMortem && (
                <Badge tone={verdictTone(t.journal.postMortem.classification)}>{t.journal.postMortem.classification.replace(/_/g, " ")}</Badge>
              )}
            </div>
